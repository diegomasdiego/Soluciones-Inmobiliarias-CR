...
export function VerifiedPanel({ p, dark }: { p: Pick<Property, 'legal'>; dark?: boolean }) {
  const d = formatDate(p.legal.checked);
  const checks = [
    { label: 'Title (Folio Real) matches the seller', meta: p.legal.folio },
    { label: 'Cadastral plan matches the land', meta: p.legal.plano },
    { label: 'No liens, annotations or lawsuits', meta: 'Registro Nacional' },
    { label: 'Municipal taxes up to date', meta: 'Municipality' },
    { label: 'Escrow-ready with a SUGEF-regulated agent', meta: 'Escrow' },
  ];
  return (
    <div className={`verified ${dark ? 'verified-dark' : ''}`}>
      <div className="verified-head">
        ...
        <div>
          ...
          <span>Checked on {d} · refreshed before closing</span>
        </div>
      </div>
      <ul>
        {checks.map((c, i) => (
          <motion.li key={c.label} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.08 * i }}>
...
export function Lightbox({ photos, title, index, onClose }: { photos: string[]; title: string; index: number | null; onClose: () => void }) {
  ...
  return (
    <AnimatePresence>
      {index != null && (
        <motion.div className="lightbox" role="dialog" aria-modal="true" aria-label={`${title} photos`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <div className="lightbox-bar">
            ...
            <button ref={closeRef} type="button" className="icon-btn" onClick={onClose} aria-label="Close photos"><IconClose /></button>
          </div>
          <div className="lightbox-stage" data-cursor="Drag">
            <AnimatePresence initial={false} custom={dir} mode="popLayout">
              ...
              >
                <Img name={photos[i]} alt={`${title}, photo ${i + 1}`} eager draggable={false} />
              </motion.div>
            </AnimatePresence>
            <button type="button" className="lightbox-nav prev" onClick={() => go(-1)} aria-label="Previous photo"><IconArrowLeft /></button>
            <button type="button" className="lightbox-nav next" onClick={() => go(1)} aria-label="Next photo"><IconArrow /></button>
          </div>
          <div className="lightbox-thumbs">
            {photos.map((ph, k) => (
              <button key={ph} type="button" className={k === i ? 'is-current' : ''} onClick={() => { setDir(k > i ? 1 : -1); setI(k); }} aria-label={`Photo ${k + 1}`}>
                <Img name={ph} alt="" />
              </button>
            ))}
          </div>
...
export function FloorPlan({ p, onOpenPhoto }: { p: Property; onOpenPhoto: (i: number) => void }) {
  ...
  return (
    <div className="plan">
      <svg viewBox="0 0 100 60" className="plan-svg" role="img" aria-label={`Floor plan of ${p.title}`}>
        <defs>
        ...
        {p.plan.rooms.map((r, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={r.x + 1.4} y={r.y + 3.4} className="plan-label">{r.name}</text>
              {r.dims && <text x={r.x + 1.4} y={r.y + 6.2} className="plan-dims">{r.dims}</text>}
            </g>
          );
        })}
